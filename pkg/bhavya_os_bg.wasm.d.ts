/* tslint:disable */
/* eslint-disable */
export const memory: WebAssembly.Memory;
export const __wbg_terminal_free: (a: number, b: number) => void;
export const terminal_get_buffer: (a: number) => [number, number];
export const terminal_get_cwd: (a: number) => [number, number];
export const terminal_handle_down: (a: number) => [number, number];
export const terminal_handle_tab: (a: number) => [number, number];
export const terminal_handle_up: (a: number) => [number, number];
export const terminal_new: () => number;
export const terminal_process_key: (a: number, b: number, c: number) => [number, number];
export const __wbindgen_externrefs: WebAssembly.Table;
export const __wbindgen_free: (a: number, b: number, c: number) => void;
export const __wbindgen_malloc: (a: number, b: number) => number;
export const __wbindgen_realloc: (a: number, b: number, c: number, d: number) => number;
export const __wbindgen_start: () => void;
